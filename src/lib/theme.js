// ===== الهوية البصرية الجديدة لميزان: الزمردي الذهبي (Emerald Gold) =====
// كبير المهندسين — MIZAN v12 | مصمم لتطبيق جوال React Native (Android + iOS)
// ملاحظة: أسماء المتغيّرات القديمة (royal/gold/navy...) محفوظة لضمان عدم انهيار
// الشاشات الحالية، لكن قيمها محدّثة لتعكس الهوية الزمردية الجديدة.

// ============ نظام الثيمات الأربعة (Theming Engine) ============
// الزمردي هو الافتراضي. بقية الثيمات تُطبّق عبر مبدّل في شاشة "حسابي".
export const THEMES = {
  emerald: {
    id: "emerald",
    name: "زمردي",
    label: "Emerald / Gold",
    g1: "#0F5132", g2: "#0A3D26", g3: "#125E3A", g4: "#08311E", // تدرّج الهيدر/الفوتر المتموّج
    primary: "#0F5132",
    primaryDeep: "#0A3D26",
    accent: "#C9A227",
    accentLite: "#E3C766",
    light: "rgba(15,81,50,0.10)",   // خلفية أيقونة المحور
    onGrad: "#FFFFFF",
    onGradDim: "rgba(255,255,255,0.85)",
  },
  midnight: {
    id: "midnight",
    name: "ليل",
...
// ============ COLORS — لوحة الهوية ============
// محدّثة للزمردي-الذهبي. الأسماء القديمة (royal/gold/navy...) محفوظة بقيم جديدة
// متوافقة، لمنع انهيار الشاشات الحالية التي تستوردها.
export const COLORS = {
  // الخلفيات
  bg: "#FBFCFA",
  bgPure: "#FFFFFF",
  ...
  // ===== أسماء التوافق القديمة (يمنع حذفها لحماية النظام) — قيم جديدة زمردية =====
  navy1: "#0F5132",
  navy2: "#0A3D26",
  navyDark: "#08311E",
  navyMid: "#0A2A1B",
...
// ============ الظلال الناعمة (Soft Drop Shadow) ============
export const SHADOW = {
  card: {
    shadowColor: "#0F5132",
...
// ============ المحاور الثمانية (Hubs) ============
// 8 محاور تجمع 23 خبيراً. كل محور يفتح قائمة خبرائه، وكل خبير له دستوره
// المعزول (Domain Isolation) في الخادم. icon = اسم FontAwesome5.
export const HUBS = [
  {
    id: "security",
    title: "الأمن والبلاغات",
    sub: "الشرطة والبلاغات",
    icon: "shield-alt",
    experts: [
...
// خريطة سريعة: من معرّف الخبير إلى بياناته + المحور الأب (لشاشة الخبير والدساتير)
export const EXPERTS_INDEX = (() => {
  const map = {};
  HUBS.forEach((hub) => {
    (hub.experts || []).forEach((ex) => {
      map[ex.id] = { ...ex, hubId: hub.id, hubTitle: hub.title };
    });
  });
  return map;
})();

// إجمالي عدد الخبراء (للعرض)
...
// الشاشات القديمة (HomeScreen/SectionScreen) تستورد SECTIONS. نشتقّها من المحاور
// حتى لا تنكسر قبل تحديثها. كل عنصر يحمل نفس شكل القديم (id/title/sub/icon).
export const SECTIONS = HUBS.map((h) => ({
  id: h.id,
  title: h.title,
  sub: h.sub,
  icon: h.icon,
